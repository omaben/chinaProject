<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="Name" min-width="150px">
        <template slot-scope="{row}">
          ...
          <!-- <el-tag style="margin-right:10px;margin-bottom:10px;cursor:pointer" v-for="type in row.type" :key="type.key">{{ type | typeFilter }}</el-tag> -->
        </template>
      </el-table-column>
      <el-table-column v-if="showReviewer" label="Sponsor Organisation" min-width="150px">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.organisation }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="Actions" align="center" width="230" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          ...
          <el-button type="primary" size="mini" title="Tasks" class="mb-1" @click="handleViewTask(row)">
            <svg-icon icon-class="list" />
          </el-button>
          <el-button type="primary" size="mini" title="Notes" class="mb-1" @click="handleUpdate(row)">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="180px" style="width: 100%;">
        ...
        <el-form-item label="Start Date" prop="timestamp">
...
<script>
import { fetchList, fetchPv, createProject, updateProject } from '@/api/project'
import waves from '@/directive/waves' // waves directive
import { parseTime } from '@/utils'
import Pagination from '@/components/Pagination' // secondary package based on el-pagination
import Teams from '../teams' // secondary package based on el-pagination

const calendarTypeOptions = [
  { key: 'T1', display_name: 'TEAM1' },
  { key: 'T2', display_name: 'TEAM2' },
  { key: 'T3', display_name: 'TEAM3' },
  { key: 'T4', display_name: 'TEAM4' }
]
const departementOptions = [
  { key: 'D1', display_name: 'DEPARTMENT1' },
  { key: 'D2', display_name: 'DEPARTMENT2' },
  { key: 'D3', display_name: 'DEPARTMENT3' },
  { key: 'D4', display_name: 'DEPARTMENT4' }
]
...
const departmentKeyValue = departementOptions.reduce((acc, cur) => {
  acc[cur.key] = cur.display_name
  return acc
}, {})
...
export default {
  ...
  data() {
    return {
      ...
      statusOptions: ['InProgress', 'Finish'],
      showReviewer: false,
      temp: {
        id: undefined,
        importance: 1,
